...
public class BeforeResetPassWord {
	...
	public BeforeResetPassWord(JFrame frame) {
		...
		infoCkBtn.addActionListener(new ActionListener() { // 비밀번호변경으로 가는 버튼
		...
		HomeBtn.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if(MemberManager.loginTrue ) {
				
				frame.getContentPane().removeAll();
				AfterLoginFrame lf = new AfterLoginFrame(frame);
				}else{
					JOptionPane.showInternalMessageDialog(null, "로그인을 먼저 하십시오.");
				}
			}
